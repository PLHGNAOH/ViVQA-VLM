...
> **W03:** numbering RQ/H đã chốt; baseline **đề xuất** tại `docs/W03/Baseline_Decision.md` (chưa CHỐT). Recipe PEFT chốt ở W06.
> Constraint: **PEFT-only** (LoRA/QLoRA). No full fine-tuning. Mọi cải tiến so với **đúng 1** baseline đã reproduce.
> Owner: SV3 (PEFT) · SV1 (baseline) · SV2 (OCR) · SV4 (eval/prototype) · Cập nhật: **2026-09-17**
>
> Scaffold note (not a second table): the English W02 scaffold marked **Qwen2.5-VL-3B** as recommended primary. The weekly table below lists Qwen2.5-VL 3B/7B with VRAM estimates; **Trạng thái** is still `_…_` cho đến khi thầy Đức xác nhận.
> TODO (W4+): confirm GPU + pin versions (`transformers` / `accelerate` / `bitsandbytes` / `peft`).
> Full rationale + starting config: `docs/02_references/Reading_Notes/SV3_Paper_Review.md` §5 / §5.2.
> Quyết định baseline (đề xuất, chưa CHỐT): `docs/W03/Baseline_Decision.md`. Run naming: `docs/00_admin/CONVENTIONS.md`.
