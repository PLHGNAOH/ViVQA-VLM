...
- **Evidence:**
  - **L2 (LLaVA-1.5, Liu et al.)** — multilinguality is documented as an *emergent* effect of text-only ShareGPT data, still error-prone on non-English (secondary in-repo review `docs/literature/SV3_Paper_Review.md §P4 §8`). ✅ qualitative / ⚠️ secondary.
...
- **Evidence:**
  - **L2 (LLaVA-1.5)** — public evidence **LoRA ≈ full fine-tuning** on 7B/13B VLMs (e.g. LoRA-13B POPE F1 = 86.7 vs full-FT 85.9), all English (`docs/literature/SV3_Paper_Review.md §P4 §8`). ⚠️ secondary — justifies PEFT-only.
  - **Supporting (outside the 10-row matrix, but read in-repo):** LoRA (Hu et al., 2022), QLoRA (Dettmers et al., 2023), VL-Adapter (Sung et al., 2022) — all English/text-only; VL-Adapter reports 4.18% params ≈ full-FT on VQA (`docs/literature/SV3_Paper_Review.md §P1–P3`). ⚠️ secondary.
...
- **Evidence:**
  ...
  - **L2 (LLaVA-1.5)** — LoRA does not increase (may reduce) POPE hallucination vs full-FT; resolution reduces hallucination (`docs/literature/SV3_Paper_Review.md §P4`). ⚠️ secondary.
